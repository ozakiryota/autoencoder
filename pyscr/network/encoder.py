...
class Encoder(torch.nn.Module):
    def __init__(self, img_height, img_width, z_dim, is_train=True):
        ...
        self.z_dim = z_dim
        self.dim_fc_in = 512 * (img_height // 32) * (img_width // 32)
        ...
        if self.z_dim > 0:
            self.fc = torch.nn.Sequential(
                torch.nn.Linear(self.dim_fc_in, self.dim_fc_in + (self.dim_fc_in - z_dim) // 2),
                torch.nn.ReLU(inplace=True),
                torch.nn.Linear(self.dim_fc_in + (self.dim_fc_in - z_dim) // 2, z_dim)
            )
